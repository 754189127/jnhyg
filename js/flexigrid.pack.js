...
(function ($) {
	...
	$.addFlex = function (t, p) {
		...
		$('thead tr:first th', g.hDiv).each(function () {
			...
			var tbwidth = $('#table-list').width();;
		    var cells = document.getElementById("table-title").rows.item(0).cells.length; 
		    var cellWidth =(tbwidth-12*cells-1)/cells; 
			this.width  = cellWidth;
			//console.log(tbwidth+'/'+cells);
			if (this.width == '') {
				this.width = 100;
			}
		...
		if ($('th', g.hDiv).length) {
			...
			g.nDiv.innerHTML = "<table cellpadding='0' id='2222222222222222222222' cellspacing='0'><tbody></tbody></table>";
			$(g.nDiv).css({
				marginBottom: (gh * -1),
				display: 'none',
				top: gtop
