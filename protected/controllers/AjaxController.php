...
class AjaxController extends BaseController{
    ...
    public function actionPeriodical(){
        $selId = isset($_POST['selId'])?intval($_POST['selId']):0;
        $list = Periodical::model()->getList();
        $optionStr = '<option value="0">请选择期数</option>';
        if($list){
            foreach($list as $v){
                 if($v['id']==$selId)
                     $optionStr.='<option selected="selected" value="'.$v["id"].'">'.$v["title"].'</option>';
                 else
                     $optionStr.='<option value="'.$v["id"].'">'.$v["title"].'</option>';
            }
        }
        echo $optionStr;
    }
